function image_feats = get_spatial_pyramid_hog_features( img, vocab, d, cell_size, levels, total_num_bins, colour_space)
    ...
    [HOG_features, locations, dim, ~] = get_hog_features(img, colour_space, cell_size);
    
    %Get minimum distances between img feats and vocab
    ...
    for L = 0 : levels
        ...
        %Get bin dimensions
        bin_width = floor(dim(1)  / bins_per_dim);
        bin_height = floor(dim(2) / bins_per_dim);
