...
%because unique() sorts them. This shouldn't really matter, though.
    ...
    parfor i = 1 : num_categories
        ...
        %Train the classifier given the classification
        [w, b] = vl_svmtrain(train_image_feats', y, LAMBDA, 'MaxNumIterations', 200000);

        %Store results in a struct in array
        svm(1, i) = struct('W', w', 'B', b);
    end
 
    %Loop over all image features
    parfor i = 1 : n
        
        %Initalise 
        confident_svm_class = 0;
        confident_svm_result = -Inf(1);
        
        %Loop over svms
        for j = 1 : 15
            
            %Get svm classification of current image in loop
            c = svm(j).W * test_image_feats(i, :)' + svm(j).B;  
            
            %Find the most confident class
            if c > confident_svm_result
                confident_svm_result = c;
                confident_svm_class = j;
            end
        end
        
        %Store predicited category in vector
        predicted_categories(i) = train_labels(confident_svm_class * 100 - 1);
